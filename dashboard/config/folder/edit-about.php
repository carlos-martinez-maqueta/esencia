...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES["imagen"]) && $_FILES["imagen"]["error"] === UPLOAD_ERR_OK) {
        ...
        $rutaDestino = "../../files/about/" . $nombreArchivoUnico;

        // Guardar la imagen en la ruta especificada
        if (move_uploaded_file($_FILES["imagen"]["tmp_name"], $rutaDestino)) {
            // Editar la imagen solo si se guardó correctamente
            $imagen = $nombreArchivoUnico;
            $result1 = Folder::editBannerAbout($imagen);
            $result1->execute();
        }
    }

    // Editar el resto de los campos
    $result2 = Folder::editFolderAbout($title, $description);
    if ($result2->execute()) {
        // Items registrado correctamente
        $response = array(
            'status' => 'success',
            'message' => 'About content edited correctly.'
        );
    } else {
        // Error al registrar Items
        $response = array(
            'status' => 'error',
            'message' => 'Error when editing about content.'
        );
    }
